...
private Bucket searchNode(int key) {
	...
	for(int i=0 ; p.stat!=Status.EMPTY && i<size ;  i++) {
		if(p.stat == Status.OCCUPIED && p.getData()==key)  //같은 데이터가 이미 존재할 경우
			return p;
 ...
 do {
     switch (menu = SelectMenu()) {
      ...
      case REMOVE :                               // 삭제
				...
				if (result !=-1) {
					System.out.print(" 데이터가 삭제되었다.");
					System.out.println(result);
				}
				else
					System.out.println("데이터가 존재하지 않았기에 삭제할 수가 없다.");
				System.out.println();
          break;
