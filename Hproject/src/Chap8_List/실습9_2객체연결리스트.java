...
class LinkedList2 {
	...
	//전달된 element를 찾을 때 comparator 객체를 사용한다 
	{
		...
		if(Search(element, cc)){
			...
			while(cc.compare(element, current.data)!=0) {
				q=current;
				current=current.link;
			}
	...
	//임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		Node2 p= first; //p라는 변수를 도입해서 각 노드를 따라 간다.
		Node2 q= null;
		while (p != null &&(cc.compare(element, p.data)==1 || cc.compare(element, p.data)==0)) { // Traverse until end or find correct position
	...
	void Merge(LinkedList2 b, Comparator<SimpleObject5> cc) {
		...
		while(p !=null && q!=null) {
		    		...
		    		if (cc.compare(q.data, p.data)==1 || cc.compare(q.data, p.data)==0) {
		    			
		    				while( p.link !=null && (cc.compare(q.data, p.link.data)==0 ||cc.compare(q.data, p.link.data)==1)) {
		    				...
		    				while(q.link !=null &&(cc.compare(q.link.data, p.data)==-1 ||cc.compare(q.link.data, p.data)==0)) {
		    					q=q.link;
		    				}
		    				tmp=q;
		    				q=q.link;
