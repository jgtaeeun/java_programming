...
class Fruit4 {
	...
	public Fruit4(String fruitName, int fruitPrice , String fruitExpire) {
		this.name = fruitName;
		this.price = fruitPrice;
		this.expire = fruitExpire;
	}
...
public class Test_실습3_7객체배열이진탐색 {
	...
	public Test_실습3_7객체배열이진탐색() {
		// TODO Auto-generated constructor stub
	}
	static void showData(String text, Fruit4[] arr) {
	...
	static void sortData (Fruit4[] arr, FruitPrice cc) {
		for (int i=0; i <arr.length-1 ;i++ ) {
			int min= i;
			for (int j=i+1 ; j<arr.length ; j++) {
				if (cc.compare(arr[min], arr[j]) >=0) {
					min=j;
				}	
			}
			Fruit4 f = arr[i];
	...
	static void sortData (Fruit4[] arr, Comparator<Fruit4> cc_price) {
		for (int i=0; i <arr.length-1 ;i++ ) {
			int min= i;
			for (int j=i+1 ; j<arr.length ; j++) {
	...
	static int binarySearch(Fruit4[] arr,Fruit4 newFruit4, Comparator<Fruit4> cc_price) {
		...
		for (int i=0; i <arr.length ;i++ ) {
			if (cc_price.compare(arr[i], newFruit4)==0) {
				return i;
			 }
	...
	public static void main(String[] args) {
		...
		System.out.println("comparator cc 객체를 사용:: ");
		Arrays.sort(arr, cc);
		...
		//교재 115 Arrays.binarySearch에 의한 검색 (이름동일한 것이 있는지)
		int result3Index = Arrays.binarySearch(arr, newFruit4, cc_name);
		System.out.println("Arrays.binarySearch([수박,880,2023-5-18]) 조회결과::" + result3Index);
